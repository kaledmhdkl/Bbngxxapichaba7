    ...
    def execute_command(self, command, *args):
        try:
            if '/lag' in command[:4]:
                ...
                room_code = args[0] if len(args) > 0 else None
                if not room_code:
                    return "No room code provided for /lag"

                print(f"[{self.account_id}] Executing /lag for room {room_code}")

                # دورة الانضمام والمغادرة
                for _ in range(100):
                    self.socket_client.send(GenJoinSquadsPacket(room_code, self.key, self.iv))
                    time.sleep(0.1)
                    self.socket_client.send(ExiT('000000', self.key, self.iv))
                    time.sleep(0.01)

                return f"/lag command executed successfully for room {room_code}"

            elif '/bngx' in command[:5]:
                # الكود القديم كما هو
                return "Original /bngx command executed"
